...
namespace uk.vroad.ucm
{
    ...
    public abstract class UaMapMesh : MonoBehaviour, LUAppState, LSimTimeSec
    {
        ...
        IEnumerator CreateMapMeshes(bool useMulti, bool useMixed)
        {
            ...
            {
                CreateNamedMeshObjects(4, goMeshLanes, LaneNamedSubMeshArray(), laneMaterials,
                    LaneMaterialIndexArray());
                yield return null;
            ...
            else // one mesh for each object for all types of object
            {
                ...
                yield return null;
                CreateMesh(16, goMeshSignalBoxes, SignalBoxSubMesh());
                yield return null;
                CreateMesh(20, goMeshSignalArrows, SignalArrowSubMeshArray());
                yield return null; // need array here, materials change with signal state
                CreateMesh(24, goMeshSignalWalks, WalkSignalSubMeshArray());
                yield return null; // need array here, materials change with signal state
                ...
                CreateMesh(31, goMeshParking, ParkingSubMesh(false));
                yield return null;
                CreateMesh(32, goMeshDropOff, ParkingSubMesh(true));
                yield return null;

                CreateMesh(36, goMeshFootpaths, FootpathSubMesh(cornerToUpper));
                yield return null;
                CreateMesh(40, goMeshCrossings, CrossingSubMesh());
                yield return null;
                CreateMesh(44, goMeshZebras, ZebraSubMesh());
                yield return null;
                ...
                CreateMesh(52, goMeshGlassBuildings, BuildingSubMesh(true));
                yield return null;
                CreateMesh(60, goMeshWater, WaterSubMesh());
                yield return null;
                CreateMesh(75, goMeshTerrainSimple, TerrainSubMesh());
                yield return null;

                CreateMesh(77, goMeshBridges, BridgesSubMesh(junctionToUpper, cornerToUpper));
                yield return null;
                CreateMesh(80, goMeshTunnels, ShouldersSubMesh(MH.Tunnel, MH.Tunnel, 0.02f));
                yield return null;
                CreateMesh(84, goMeshShoulders, ShouldersSubMesh(MH.Kerbing, MH.Bridge));
                yield return null;
                CreateMesh(88, goMeshEmbankments, ShouldersSubMesh(MH.Embankment, MH.Embankment));
                yield return null;
                CreateMesh(90, goMeshBarriers, BarriersSubMesh());
                yield return null;
                CreateMesh(92, goMeshMedians, MedianStripSubMesh());
                yield return null;
        ...
        private void CreateLayeredTerrainData(int progress)
        {
            ...
            terrainLayer.name = SF.TERRAIN + SC.UL + App().Map().GetSuburb();
        ...
        public string SaveMapMesh()
        {
            ...
            try
            {
                ...
                foreach (GameObject goMesh in listOfGoMeshes)
                {
                    ...
                    {
                         SaveMesh(meshPath, goMesh, goMesh.name, optimize);
                    }
                }
